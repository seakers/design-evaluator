...
;   (assert (SYNERGIES::cross-registered (measurements GRAVITY1 GRAVITY2 GRAVITY3 GRAVITY4 GRAVITY5) (degree-of-cross-registration instrument) (platform ?id)))
;   (modify ?this (measurement-ids GRAVITY1 GRAVITY2 GRAVITY3 GRAVITY4 GRAVITY5)))
...
;(defrule SYNERGIES::add-sensitivity-in-tropo
;
;    ?no <- (REQUIREMENTS::Measurement (Parameter ?p) (sensitivity-in-low-troposphere-PBL Low) (Id ?id1) (taken-by ?ins1))
;    (REQUIREMENTS::Measurement (Parameter ?p) (sensitivity-in-low-troposphere-PBL High) (Id ?id2) (taken-by ?ins2))
;    (or (test (meas-group ?p 1.0.0)) (test (meas-group ?p 2.0.0)))
;    (SYNERGIES::cross-registered (measurements $?m))
;    (test (member$ ?id1 $?m))
;    (test (member$ ?id2 $?m))
;    =>
;    (duplicate ?no (sensitivity-in-low-troposphere-PBL High) (Id (str-cat ?id1 "-syn-" ?id2 ))
;         (taken-by (str-cat ?ins1 "-syn-" ?ins2)) )
;    )
